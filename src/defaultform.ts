...
if(!component.isDefined("hr.defaultform")){
    var builder = new ComponentBuilder(
        '<div class="form-group" data-hr-toggle="{{buildName}}Error" data-hr-class-on="has-error"><label class="control-label">{{title}}</label><input class="form-control" name="{{buildName}}" type="{{buildType}}" /></div>'
    );
    ...
    builder.addVariant("select", new VariantBuilder(
        '<div class="form-group" data-hr-toggle="{{buildName}}Error" data-hr-class-on="has-error"><label class="control-label">{{title}}</label><select class="form-control" name="{{buildName}}"/></div>'
    ));
    builder.addVariant("multiselect", new VariantBuilder(
        '<div class="form-group" data-hr-toggle="{{buildName}}Error" data-hr-class-on="has-error"><label class="control-label">{{title}}</label><select class="form-control" name="{{buildName}}" multiple size="{{size}}"/></div>'
    ));
